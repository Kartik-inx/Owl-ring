...
export default function HealthIntelligence({ currentChapter, isMobile = false }: HealthIntelligenceProps) {
  const show = isMobile || currentChapter === 4;
  ...
  const content = (
    <motion.div 
      ...
      className="max-w-[480px] w-full cinematic-blur-panel border border-white/10 p-6 sm:p-10 rounded-[24px] sm:rounded-[32px] flex flex-col gap-6 sm:gap-8 relative pointer-events-auto bg-neutral-900/30 backdrop-blur-xl shadow-2xl"
    >
      ...
      <motion.div 
        ...
        whileInView={isMobile ? { opacity: 1, scaleX: 1, transition: { duration: 0.8, ease: "easeOut" } } : {}}
        viewport={{ once: true }}
      ...
      >
        ...
        <div className="flex justify-between items-center relative z-10">
          ...
          <div className="text-right">
            <span className="text-2xl sm:text-3xl font-semibold text-white tracking-tight block">07:00 <span className="text-xs sm:text-sm text-neutral-500 font-normal">AM</span></span>
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <motion.div 
          ...
          whileInView={isMobile ? { opacity: 1, y: 0, transition: { duration: 0.8, ease: "easeOut" } } : {}}
          viewport={{ once: true }}
          className="bg-white/5 rounded-2xl p-4 sm:p-5 border border-white/5 flex flex-col justify-between h-[110px] sm:h-[120px] group hover:border-white/10 transition-colors cursor-default text-left"
        >
          ...
          <div>
            <span className="text-2xl sm:text-3xl font-semibold text-white tracking-tight">92</span>
            <p className="text-[10px] sm:text-xs text-blue-400 font-medium mt-1">Uninterrupted Rest</p>
          </div>
        </motion.div>
        ...
        >
          ...
          <div>
            <span className="text-2xl sm:text-3xl font-semibold text-white tracking-tight">94%</span>
            <p className="text-[10px] sm:text-xs text-emerald-400 font-medium mt-1">Primed for the day</p>
          </div>
        </motion.div>
      </div>
