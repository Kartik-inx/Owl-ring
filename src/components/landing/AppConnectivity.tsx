...
export default function AppConnectivity({ currentChapter, isMobile = false }: AppConnectivityProps) {
  const show = isMobile || currentChapter === 6;
  ...
  const content = (
    <motion.div 
      ...
      className={`max-w-[480px] w-full flex flex-col gap-6 sm:gap-8 pointer-events-auto ${isMobile ? "text-center items-center" : "text-left lg:text-right items-start lg:items-end"}`}
    >
      <motion.div
        initial={isMobile ? { opacity: 0, y: 15 } : {}}
        whileInView={isMobile ? { opacity: 1, y: 0, transition: { duration: 0.8, ease: "easeOut" } } : {}}
        viewport={{ once: true }}
        className={isMobile ? "flex flex-col items-center" : "flex flex-col items-start lg:items-end"}
      >
      ...
      <motion.div 
        ...
        whileInView={isMobile ? { opacity: 1, y: 0, transition: { duration: 0.8, ease: "easeOut" } } : {}}
        viewport={{ once: true }}
        className="bg-neutral-900/30 border border-white/10 rounded-3xl p-6 relative overflow-hidden mt-4 shadow-2xl group hover:border-white/20 transition-all duration-300 w-full"
      >
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div className="text-left">
            ...
            <p className="text-neutral-400 text-xs font-light">BLE Active Sync</p>
          </div>
          <Wifi className="text-blue-500 ml-auto animate-pulse" size={20} />
        </div>

        <div className="space-y-3">
          <div className="h-10 rounded-xl bg-white/5 border border-white/5 flex items-center px-4 justify-between hover:bg-white/10 transition-colors">
