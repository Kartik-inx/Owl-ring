...
export default function FaqCta({ currentChapter, isMobile = false }: FaqCtaProps) {
  const [openId, setOpenId] = useState<number | null>(0);
  const show = isMobile || currentChapter === 10;
  ...
  const content = (
    <div className={`max-w-5xl w-full mx-auto flex flex-col pointer-events-auto ${isMobile ? "gap-12 px-4 py-8 items-center" : "md:h-auto px-4 sm:px-6 py-20 md:py-0 md:flex-row gap-8 sm:gap-16 md:gap-24 items-center md:justify-between justify-start overflow-y-auto md:overflow-visible pointer-events-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"}`}>
      ...
      <motion.div 
        ...
        className={`w-full ${isMobile ? "max-w-xl text-center" : "md:w-1/2 mt-8 md:mt-0"}`}
      >
        <h3 className={`text-base sm:text-lg font-semibold text-white uppercase tracking-widest mb-6 sm:mb-8 text-shadow-cinematic opacity-80 ${isMobile ? "text-center" : "text-left"}`}>Frequently Asked Questions</h3>
          ...
          {faqs.map((faq, index) => {
            ...
            return (
              <motion.div 
                ...
                whileInView={isMobile ? { opacity: 1, y: 0, transition: { duration: 0.5, delay: index * 0.05 } } : {}}
                viewport={{ once: true }}
                key={index} 
                className="border-b border-white/10 py-4 sm:py-5 cursor-pointer group"
                onClick={() => setOpenId(isOpen ? null : index)}
              >
                <div className="flex justify-between items-center">
                  <h4 className={`text-xs sm:text-sm md:text-base font-medium transition-colors ${isOpen ? "text-[#c6a972]" : "text-white group-hover:text-neutral-300"}`}>
                    {faq.q}
                  </h4>
                  <ChevronDown size={16} className={`transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] flex-shrink-0 ml-4 ${isOpen ? "rotate-180 text-[#c6a972]" : "text-neutral-600 group-hover:text-white"}`} />
                </div>
                <div className={`grid transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${isOpen ? "grid-rows-[1fr] opacity-100 mt-2 sm:mt-4" : "grid-rows-[0fr] opacity-0 mt-0"}`}>
                  <div className="overflow-hidden">
                    <p className="text-neutral-400 text-xs sm:text-sm font-light leading-relaxed pr-6 sm:pr-8 pb-2 sm:pb-0">{faq.a}</p>
                  </div>
                </div>
              </motion.div>
            );
      ...
      <motion.div 
        initial={isMobile ? { opacity: 0, y: 15 } : { opacity: 0, x: 30 }}
        whileInView={isMobile ? { opacity: 1, y: 0, transition: { duration: 0.8, ease: "easeOut", delay: 0.1 } } : {}}
        viewport={{ once: true, margin: "-50px" }}
        className={`w-full ${isMobile ? "max-w-xl flex flex-col items-center text-center pb-12" : "md:w-1/2 flex flex-col items-center md:items-start text-center md:text-left pb-12 md:pb-0"}`}
      >
        ...
        <h2 className="text-xl sm:text-2xl md:text-4xl font-medium uppercase tracking-wide text-white text-shadow-cinematic mb-4 sm:mb-6 leading-tight">
          WAKE UP<br />TO SOMETHING<br />BETTER.
        </h2>
        <p className="text-premium-body text-[11px] sm:text-sm md:text-base mb-8 sm:mb-10 text-shadow-cinematic">
          Reserve your Owl Ring today. Titanium-forged, sleep-engineered, and ready to redefine your mornings.
        </p>
        
        <Link href="/product/owl-ring-s1" className="group relative inline-flex items-center justify-center gap-3 bg-white text-black px-8 sm:px-10 py-4 sm:py-5 rounded-full overflow-hidden w-full sm:w-auto shadow-[0_0_30px_rgba(255,255,255,0.15)] hover:shadow-[0_0_40px_rgba(198,169,114,0.3)] transition-all duration-300 hover:scale-105">
  ...
  if (isMobile) {
    return (
      <section className="relative w-full py-16 px-4 sm:px-6 z-20 flex flex-col items-center justify-center pointer-events-auto bg-neutral-950/20">
        {content}
      </section>
    );
  }
